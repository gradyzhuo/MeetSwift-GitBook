...
而在 Swift 1.2 ，導入了 `??` 這個語法來簡化 Unwrapping 的過程。
...
`??` 跟 Objective-C 的 `nil` 呼叫 method 的方式有些相似，但不太一樣的是 Objective-C 是不知道什麼時候會 `nil` ， 但 Swift 的 `??` 是因為你知道這裡會有 `nil` 的可能才進行合理性賦值的動作，換句話說，這個 `nil` 的處理，是在預料之中的。

## Optional  Chaining 一條鞭法 
要講 Optional Chaining 之前，我們先考量以下常見的的狀況：
...
有一個重點是，**Optional Chaining** 所取得的值，都一定是 `Optional` ，所以也可以想像 **Optional Chaining** 是在 `Optional` 裡面去**拆包裏**，所以就可以避開 Crash 。
